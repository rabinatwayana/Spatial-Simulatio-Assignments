...
species vierkher {
	...
	aspect vierkher_polygon_aspect {
		draw vierkaser_polygon color: #grey border: #black;
	}
...
species cleaned_2021 {
	...
	aspect cleaned_2021_polygon_aspect {
		draw cleaned_2021_polygon color: #yellow border: #black;
	}
...
species cleaned_2022 {
	...
	aspect cleaned_2022_polygon_aspect {
		draw cleaned_2022_polygon color: #yellow border: #black;
	}
...
species cleaned_2023 {
	...
	aspect cleaned_2023_polygon_aspect {
		draw cleaned_2023_polygon color: #yellow border: #black;
	}

}

species cows skills: [moving] {
	float action_radius <- 50.0 #m;
	geometry cow_action_area;

	reflex move_cow {
		do wander speed: 50.0 amplitude: 90.0 bounds: grassland;
	}
	...
	reflex cow_graze {
		...
		ask my_grass {
			biomass <- biomass - 0.4; //biomass is a grid variable in this example
		}
...
//declare variables
	...
	reflex grow when: is_pasture {
		if (is_meadow and biomass < 6) or (is_cutback_2020 and biomass < 7) or (is_cutback_2021_23 and biomass < 6) or (is_hirschanger and biomass < 4) {
			biomass <- biomass + 0.1;
		}
	}
...
//			draw square(5) color: rgb(0, int(biomass * 40), 0) border: rgb(0, int(biomass * 40), 0);
			draw square(5) color: rgb(0, 255-int(biomass * 40), 0) border: rgb(0, 255-int(biomass * 40), 0);
		}
